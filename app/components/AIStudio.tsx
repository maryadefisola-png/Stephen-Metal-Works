...
export default function AIStudio() {
  ...
  return <div className="ai-studio-live">
    ...
    <div className="ai-result">{concept ? <><div className="sectiontag">AI CONCEPT</div><h3>{concept.concept_name}</h3><p>{concept.customer_summary}</p><h4>Design features</h4><ul>{(concept.design_features || []).map((x: string, i: number) => <li key={i}>{x}</li>)}</ul><h4>Practical notes</h4><p>{concept.practical_notes}</p><button className="button light" onClick={() => document.getElementById('quote')?.scrollIntoView({ behavior: 'smooth' })}>Request a Quote for This Design ↗</button></> : <div className="ai-empty"><span>✦</span><h3>Your concept will appear here</h3><p>Describe what you imagine. The assistant will turn it into a clear design direction for Stephen Metal Works.</p></div>}</div>
  </div>
}
